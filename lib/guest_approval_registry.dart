...
class GuestApprovalRegistry {
  ...
  GuestApprovalBeginResult begin({
    ...
    _log(
      ...
      details:
          'reason=no_existing_entry_for_key connectionAttemptId=$connectionAttemptId '
          'guestPeerId=$guestPeerId source=${source.label}',
    );
  ...
  GuestApprovalBeginResult _reuse(
    ...
    _maybeUpdateDisplayName(existing, displayName);
    ...
    if (existing.isApproved && existing.sessionActive) {
      _log(
        ...
        details:
            ...
            'name=${existing.displayName} token=${_tokenPreview(existing.accessToken)}',
      );
      _log(
        'Approval | Approved entry metadata updated',
        details:
            'key=${existing.key.value} displayName=${existing.displayName} '
            'source=${source.label}',
      );
      return GuestApprovalBeginResult(
    ...
    _log(
      kind == GuestApprovalOutcomeKind.ignoredDuplicate
          ? 'Approval | Duplicate approval suppressed'
          : 'Approval | Existing approval reused',
      details:
          'key=${existing.key.value} source=${source.label} '
          'originalSource=${existing.source.label} name=${existing.displayName} '
          'kind=${kind.name}',
    );
  ...
  GuestApprovalBeginResult _attachAndReuse(
    ...
    _maybeUpdateDisplayName(bridged, displayName);
    _log(
  ...
  void _maybeUpdateDisplayName(GuestApprovalEntry entry, String displayName) {
    final incoming = displayName.trim();
    if (incoming.isEmpty) return;
    final current = entry.displayName.trim();
    if (current == incoming) return;
    final currentIsPlaceholder =
        current.isEmpty || _isPlaceholderName(current.toLowerCase());
    final incomingIsPlaceholder = _isPlaceholderName(incoming.toLowerCase());
    // Prefer a real name over a placeholder; never wipe a real name with a placeholder.
    if (!currentIsPlaceholder && incomingIsPlaceholder) return;
    entry.displayName = incoming;
    _log(
      'Approval | displayName updated in existing entry',
      details:
          'key=${entry.key.value} previous=$current displayName=$incoming',
    );
  }
