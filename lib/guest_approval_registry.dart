...
/// Issues an auth token for an approved guest display name.
typedef GuestApprovalTokenFactory = String Function(String displayName);

/// Called when a previously issued token is revoked (leave / invalidate / clear).
typedef GuestApprovalTokenRevoked = void Function(String token);
...
///
/// Token issuance is pluggable so the hub can bind grants to [HubAuth] (TLS
/// session tokens) while keeping attempt-scoped lifetime here.
class GuestApprovalRegistry {
  GuestApprovalRegistry({
    GuestApprovalLog? log,
    DateTime Function()? now,
    GuestApprovalTokenFactory? tokenFactory,
    GuestApprovalTokenRevoked? onTokenRevoked,
  })  : _log = log ?? ((_, {details}) {}),
        _now = now ?? DateTime.now,
        _tokenFactory = tokenFactory ?? _defaultToken,
        _onTokenRevoked = onTokenRevoked;

  final GuestApprovalLog _log;
  final DateTime Function() _now;
  final GuestApprovalTokenFactory _tokenFactory;
  final GuestApprovalTokenRevoked? _onTokenRevoked;
  ...
  GuestAccessGrant? resolve({
    ...
    final token = _tokenFactory(entry.displayName);
  ...
  static String _defaultToken(String displayName) {
